...
Cookpad.Recipe = function() {
	this.initialize.apply(this, arguments);
};
Cookpad.Recipe.prptotype = {
	_date: null,
	_index: -1,
	_id: null,
	
	initialize: function(date, index, id) {
		this._date = date;
		this._index = index;
		this._url = id;
	},
	request: function() {
		var option = {
			host: 'cookpad.com',
			path: this._url.replace('http://cookpad.com', '').replace('/recipe/', '/recipe/print/')
		};
	}
};
...
http.createServer(function (request, response) {
	...
	var cookpad = new Cookpad(request, response, (days && days.match(/[1-3]/)) ? days : 1);
	cookpad.request();
